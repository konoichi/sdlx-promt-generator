...
def get_active_addons(user_capabilities: list[str]) -> List[Addon]:
    ...
    for addon in _INSTALLED_ADDONS.values():
        ...
        if not req_key or is_feature_unlocked(req_key, user_capabilities):
            active.append(addon)
    return active

def get_active_addons_by_type(addon_type: Type[T], user_capabilities: list[str]) -> List[T]:
    """Gibt alle aktiven Addons eines bestimmten Typs für einen Nutzer zurück."""
    return [a for a in get_active_addons(user_capabilities) if isinstance(a, addon_type)]

def list_all_installed_addons(user_capabilities: list[str]) -> List[dict]:
    ...
    for aid, a in _INSTALLED_ADDONS.items():
        ...
        is_active = not req_key or is_feature_unlocked(req_key, user_capabilities)
        addons.append({
            "id": a.id,
            "name": a.name,
